...
function noteNameToMidi(name, octave) {
  return Audio.noteToMidi(name, octave);
}

function midiToHz(midi) {
  return Audio.midiToHz(midi);
}
...
function buildChordGrid() {
  ...
  grid.innerHTML = '';
  chords.forEach(({ label, freqs, noteName, quality }) => {
...
function buildMelodyGrid() {
  ...
  // Affiche les notes de la gamme sur 2 rangées de 12
  notes.slice(0, 24).forEach((freq, i) => {
    const btn = document.createElement('button');
    btn.className = 'melody-btn';
    // Retrouver le nom de la note
  ...
  // Mise à jour de la liste de notes pour le séquenceur
...
function startArpeggio(freqs) {
  ...
  state.arpInterval = setInterval(() => {
    ...
    switch (state.arp) {
      ...
      case 'updown':
        idx = arpIndex % arpFreqs.length;
        if (arpIndex >= arpFreqs.length - 1) arpDir = -1;
        if (arpIndex <= 0) arpDir = 1;
        arpIndex += arpDir;
        break;
      case 'random': idx = Math.floor(Math.random() * arpFreqs.length); break;
    }
    Audio.playNote(arpFreqs[idx], 60 / state.bpm * 0.4);
  }, interval);
}
...
async function exportMP3() {
  ...
  try {
    // Décoder le webm en PCM via AudioContext offline
    ...
    // lamejs encode en MP3 320 kbps
    const mp3enc = new lamejs.Mp3Encoder(2, sampleRate, 320);
    const blockSize = 1152;
    const mp3Data   = [];

    for (let i = 0; i < left.length; i += blockSize) {
      const lSlice = toInt16(left.subarray(i, i + blockSize));
      const rSlice = toInt16(right.subarray(i, i + blockSize));
      const chunk  = mp3enc.encodeBuffer(lSlice, rSlice);
    ...
    a.href        = url;
    a.download    = `pulsechord_${Date.now()}.mp3`;
    a.click();
    ...
    // Fallback: télécharger le webm brut
    const url = URL.createObjectURL(state.recBlob);
    const a   = document.createElement('a');
    a.href    = url;
    a.download = `pulsechord_${Date.now()}.webm`;
    a.click();
    URL.revokeObjectURL(url);
...
function applyPreset(name) {
  ...
  Audio.setParam('waveform',   p.waveform);
  Audio.setParam('attack',     p.attack);
  Audio.setParam('release',    p.release);
  Audio.setParam('filter',     p.filter);
  Audio.setParam('resonance',  p.resonance);
  Audio.setParam('reverb',     p.reverb);
  Audio.setParam('delay',      p.delay);
  Audio.setParam('distortion', p.distortion);
  Audio.setParam('bitcrush',   p.bitcrush);
  Audio.setParam('volume',     p.volume);
  ...
  // Sync UI
  setSelectVal('root-note', p.root);
  setSelectVal('scale',     p.scale);
  setSelectVal('arp-mode',  p.arp);
  syncSlider('bpm',        p.bpm);
  syncSlider('attack',     p.attack);
  syncSlider('release',    p.release);
  syncSlider('filter',     p.filter);
  syncSlider('resonance',  p.resonance);
  syncSlider('reverb',     p.reverb);
  syncSlider('delay-mix',  p.delay);
  syncSlider('distortion', p.distortion);
  syncSlider('bitcrush',   p.bitcrush);
  syncSlider('volume',     p.volume);
  ...
  document.querySelectorAll('[data-engine]').forEach(b =>
    b.classList.toggle('active', b.dataset.engine === p.engine));
  document.querySelectorAll('[data-wave]').forEach(b =>
    b.classList.toggle('active', b.dataset.wave === p.waveform));
  ...
  if (active) { active.classList.add('active'); setTimeout(() => active.classList.remove('active'), 1200); }
}
...
function init() {
  ...
  // Charger lamejs depuis CDN si pas déjà présent
